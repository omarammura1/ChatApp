...
import com.oateam.chat.Utilities.PDF;
...
public class ToolsPDF {
    ...
    public static void openInAppBrowserPDF(Activity activity, String url, boolean from_notif) {
        url = appendQuery(url, "t=" + System.currentTimeMillis());
        if (!URLUtil.isValidUrl(url)) {
            Toast.makeText(activity, "Ops, Cannot open url", Toast.LENGTH_LONG).show();
            return;
        }
        PDF.navigate(activity, url, from_notif);
    }
